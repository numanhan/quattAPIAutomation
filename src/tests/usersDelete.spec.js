    ...
    if (!randomUserId || !unauthTestUserId) {
        ...
        unauthTestUserId =
            remainingUsers[Math.floor(Math.random() * remainingUsers.length)]
                .id;
        console.log(
            `Selected Random User ID for Unauthorised Test: ${unauthTestUserId}`
        );
    }
}
...
test.describe('API Endpoint: Delete User', () => {
    ...
    test('Delete User - Unauthorised Test', async ({ baseURL }) => {
        await setupUsers(baseURL);
        console.log(
            `Running Unauthorised Test with User ID: ${unauthTestUserId}`
        );
    ...
    test('Delete User - Negative Test (Invalid User)', async ({ baseURL }) => {
        ...
        console.log(
            `Running Negative Test with Invalid User ID: ${invalidUserId}`
        );
